...
class Solution {
 ...
 private:
  ...
  TreeNode *buildTree(vector<int> &preorder, int preIndex, vector<int> &inorder,
    ...
    // find A in inorder
    int lenB = 0;
    while (inorder[inIndex + lenB] != preorder[preIndex]) ++lenB;
    // decode B, C
    root->left = buildTree(preorder, preIndex + 1, inorder, inIndex, lenB);
    root->right = buildTree(preorder, preIndex + 1 + lenB, inorder,
                            inIndex + 1 + lenB, size - 1 - lenB);
    return root;
  }
};
