...
class Solution {
 public:
  vector<vector<string> > findLadders(string start, string end,
    ...
    while (!curLevel.empty() && !found) {
      ++level;
      for (set<string>::const_iterator it = curLevel.begin();
           it != curLevel.end(); ++it) {
        ...
        for (int i = it->length() - 1; i >= 0; --i) {
          ...
          for (char c = 'a'; c <= 'z'; ++c) {
            ...
            if (dict.find(oneEditDistanceWord) == dict.end()) continue;
            mySuccessor.insert(oneEditDistanceWord);
            nextLevel.insert(oneEditDistanceWord);
            if (oneEditDistanceWord == end) found = true;
          }
        }
        successors.insert(make_pair(*it, mySuccessor));
      }
      for (set<string>::const_iterator it = nextLevel.begin();
           it != nextLevel.end(); ++it)
        dict.erase(*it);
 ...
 private:
  void writeResult(map<string, set<string> >& successors, string& end,
                   vector<string>& prefix, vector<vector<string> >& result,
                   int level) {
    ...
    for (set<string>::const_iterator it = mySuccessor.begin();
         it != mySuccessor.end(); ++it) {
      prefix.push_back(*it);
