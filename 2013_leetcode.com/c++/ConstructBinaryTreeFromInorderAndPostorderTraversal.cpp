...
class Solution {
 ...
 private:
  ...
  TreeNode *buildTree(vector<int> &inorder, int inIndex, vector<int> &postorder,
                      int postIndex, int size) {
    ...
    // find A in inorder
    int lenB = 0;
    while (inorder[inIndex + lenB] != A) ++lenB;
    // decode B, C
    root->left = buildTree(inorder, inIndex, postorder, postIndex, lenB);
    root->right = buildTree(inorder, inIndex + 1 + lenB, postorder,
                            postIndex + lenB, size - 1 - lenB);
    return root;
  }
};
