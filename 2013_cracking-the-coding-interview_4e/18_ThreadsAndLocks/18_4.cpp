...
/*
 * In order for a deadlock to occur, must have the following four conditions
 * met:
 ...
 * 3. No Preemption: One process cannot forcibly remove another process’
 * resource.
...
void* AThreadBreakCond4(void* para) {
  ...
  for (vector<int>::const_iterator it = requiredResIds.begin();
       it != requiredResIds.end(); ++it)
    Resource::acquire(*it);

  // do something

  for (vector<int>::reverse_iterator it = requiredResIds.rbegin();
       it != requiredResIds.rend(); ++it)
    Resource::release(*it);
  return NULL;
}
