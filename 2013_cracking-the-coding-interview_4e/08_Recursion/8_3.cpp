...
// from front to back
template <class T>
void getSubSets(const set<T>& source, typename set<T>::iterator currentPos,
                set<T>& subset) {
  if (currentPos == source.end()) {
    cout << "{";
    for (typename set<T>::iterator it = subset.begin(); it != subset.end();
         ++it)
      cout << *it << ", ";
    cout << "}" << endl;
    return;
...
// from back to front
template <class T>
void getSubSets2(const set<T>& source, typename set<T>::iterator currentPos,
...
int main() {
  ...
  for (vector<set<int> >::iterator it = subsets.begin(); it != subsets.end();
       ++it) {
    cout << "{";
