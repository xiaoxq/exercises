...
void printXQueensSolutions(int row) {
  ...
  for (int col = 0; col < BoardWidth; ++col) {
    ...
    if (marked_col[col] || marked_backslash[row + col] ||
        marked_slash[BoardWidth - 1 + row - col])
      continue;
    // mark, try, and revert
    board[row][col] = marked_col[col] = marked_backslash[row + col] =
        marked_slash[BoardWidth - 1 + row - col] = true;
    printXQueensSolutions(row + 1);
    board[row][col] = marked_col[col] = marked_backslash[row + col] =
        marked_slash[BoardWidth - 1 + row - col] = false;
  }
}
