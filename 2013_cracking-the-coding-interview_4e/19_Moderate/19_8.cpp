...
string trim(const string& word) {
  ...
  if (start >= 0 && start < stop) return word.substr(start, stop);
  return word;
}

// assume: all words have been trimed
void countWord(map<string, int>& wordCount, const vector<string>& book) {
  for (vector<string>::const_iterator it = book.begin(); it != book.end();
       ++it) {
    string word(trim(*it));
    // to lower
...
int main() {
  ...
  cout << "getFrequency(love) = " << getFrequency(wordCount, "love")
       << endl;                                                            // 2
