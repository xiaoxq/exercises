...
class File {
 public:
  ...
  void write(const char* buffer, int length) {
    ...
    // init
    if (data == NULL) data = new DataBlock();
  ...
  int read(char* buffer, int pos, int maxLength) {
    // invalid
    if (pos < 0 || maxLength < 0 || data == NULL) return 0;
    ...
    // invalid
    if (pos > block->currentLength) return 0;

    int remainLength = maxLength;
    while (remainLength > 0) {
      // read
      int readLength = block->currentLength - pos;
      if (readLength > remainLength) readLength = remainLength;
...
class Dir {
 public:
  ~Dir() {
    for (map<string, Dir*>::iterator it = subDirs.begin(); it != subDirs.end();
         ++it)
      delete it->second;
    for (map<string, File*>::iterator it = subFiles.begin();
         it != subFiles.end(); ++it)
      delete it->second;
  }
...
class FileSystem {
 public:
  ...
  Dir* getDir(const vector<string>& path) {
    Dir* targetDir = root;
    for (vector<string>::const_iterator it = path.begin(); it != path.end();
         ++it) {
  ...
  File* getFile(const vector<string>& path, const string& name) {
    ...
    if (targetDir) return targetDir->getSubFile(name);
    return NULL;
  }
  void list(const vector<string>& path, map<string, Dir*>& dirs,
  ...
  void remove(const vector<string>& path, const string& name) {
    Dir* targetDir = getDir(path);
    if (targetDir) targetDir->removeChild(name);
  }
  Dir* createDir(const vector<string>& path) {
    ...
    for (vector<string>::const_iterator it = path.begin(); it != path.end();
      ...
      if (child == NULL) child = targetDir->createDir(*it);
      targetDir = child;
    }
    return targetDir;
  }
...
void printDir(FileSystem& fs, const vector<string>& path) {
  ...
  for (map<string, File*>::iterator it = subFiles.begin(); it != subFiles.end();
       ++it)
