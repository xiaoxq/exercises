...
const vector<int>& searchTinS(Node* root, const string& strT) {
  ...
  while (root != NULL) {
    ...
    for (; i < root->data.length() && i < prefix.length(); ++i)
      if (root->data[i] != prefix[i]) return EmptyIntVec;
    // strT is my prefix
    if (i == prefix.length()) return root->startPoss;
...
void test(Node* root, const string& strS, const string& strT) {
  ...
  if (startPos.empty())
    ...
    int start = 0;
    for (vector<int>::const_reverse_iterator it = startPos.rbegin();
         it != startPos.rend(); ++it) {
