...
bool isCompoundWord(const vector<string>& words,
                    map<string, bool>& compoundMark, const string& word) {
  map<string, bool>::iterator it = compoundMark.find(word);
  // has been checked
  if (it != compoundMark.end()) return it->second;

  // parse
  for (int i = word.length() - 1; i > 0; --i) {
    string leftPart = word.substr(0, i);
    bool isLeftPartAWord = binary_search(words.begin(), words.end(), leftPart,
                                         strLenReverseCompare) ||
                           isCompoundWord(words, compoundMark, leftPart);
    if (!isLeftPartAWord) continue;

    string rightPart = word.substr(i);
    bool isRightPartAWord = binary_search(words.begin(), words.end(), rightPart,
                                          strLenReverseCompare) ||
                            isCompoundWord(words, compoundMark, rightPart);
    if (!isRightPartAWord) continue;
...
string findLongestCompoundWord(vector<string>& words) {
  ...
  for (vector<string>::const_iterator it = words.begin(); it != words.end();
       ++it)
    if (isCompoundWord(words, compoundMark, *it)) return *it;
  return "";
}
...
int main() {
  string words[] = {
      ...
      "testingtester", "testtestertestin", "testtestertesting"};
  vector<string> wordsVec(words, words + 6);
  cout << findLongestCompoundWord(wordsVec) << " is the longest compound word"
       << endl;  // testingtester
  vector<string> wordsVec2(words, words + 7);
  cout << findLongestCompoundWord(wordsVec2) << " is the longest compound word"
       << endl;  // testtestertesting
  return 0;
}
