...
bool findInSortedMatrix(const vector<vector<int> >& matrix, int target,
                        int left, int right, int top, int bottom, int& resultI,
                        int& resultJ) {
  // reach the end
  ...
  if (findInSortedMatrix(matrix, target, middleJ, right, top, middleI, resultI,
                         resultJ) ||
      findInSortedMatrix(matrix, target, left, middleJ, middleI, bottom,
                         resultI, resultJ))
    return true;
  // bottom right block
  if (target >= matrix[middleI][middleJ])
    return findInSortedMatrix(matrix, target, middleJ, right, middleI, bottom,
                              resultI, resultJ);
  // top left block
  return findInSortedMatrix(matrix, target, left, middleJ, top, middleI,
                            resultI, resultJ);
}
...
#include <stdio.h>
int main() {
  int values[] = {1,  3,  5,  7,  9,  2,  4,  6,  8,  10, 11, 12, 13,
