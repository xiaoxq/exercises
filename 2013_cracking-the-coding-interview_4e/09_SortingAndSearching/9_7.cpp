...
int findLongestTower(const vector<Person>& people, vector<int>& counted_table,
                     int cur, int preHeight, int preWeight) {
  if (cur == people.size()) return 0;

  // without the current person
  int maxi =
      findLongestTower(people, counted_table, cur + 1, preHeight, preWeight);

  // with the current
  if (preHeight < people[cur].height && preWeight < people[cur].weight) {
    if (counted_table[cur] == 0)
      counted_table[cur] =
          1 + findLongestTower(people, counted_table, cur + 1,
                               people[cur].height, people[cur].weight);
    if (maxi < counted_table[cur]) maxi = counted_table[cur];
  }
...
#include <stdio.h>
int main() {
  Person people[] = {{1, 1}, {4, 4}, {2, 5}, {5, 2}, {2, 4}, {3, 5}, {4, 6}};
