...
string twoWayMerge(vector<string>& chunkfiles) {
  ...
  if (chunkfiles.size() > 0) return chunkfiles[chunkfiles.size() - 1];
  return "";
}
...
void genRandomStringFile(const char* filename, unsigned int lineSize) {
  ...
  while (lineSize--) {
    ...
    while (length == 0) length = rand() % lineLen;
    while (length--) {
      // random char
...
int main(int argc, char* argv[]) {
  ...
  if (lineSize <= 0) lineSize = 1 << 24;  // (averagely 32B/line) * (16M lines)
