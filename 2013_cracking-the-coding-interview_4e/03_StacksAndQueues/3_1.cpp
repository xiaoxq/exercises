...
class MultiStacks {
 ...
 public:
  MultiStacks(int memSize, int stackNum) {
    ...
    // init stack tops
    for (int i = 0; i < stackNum; ++i) mStackTop[i] = NULL;
  ...
  void push(int stackIndex, MemElement data) {
    if (stackIndex >= mStackNum || mFreeList == NULL) return;
  ...
  MemElement top(int stackIndex) {
    if (stackIndex >= mStackNum || mStackTop[stackIndex] == NULL) return -1;
    return *(mStackTop[stackIndex]->data);
  }

  void pop(int stackIndex) {
    if (stackIndex >= mStackNum || mStackTop[stackIndex] == NULL) return;
