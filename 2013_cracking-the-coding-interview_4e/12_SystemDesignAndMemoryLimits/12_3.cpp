...
int findAbsenceIn4BillionWith10MMemory(const char* filename) {
  ...
  // mark all present bits
  int integer;
  while (fin >> integer) ++count[integer / BucketSize];
  fin.close();
  ...
  while (fin2 >> integer)
    if (integer / BucketSize == missingIntBlock) container.push_back(integer);
  fin2.close();
  ...
  for (int i = 0; i < size; ++i)
    if (container[i] != startVal + i) return startVal + i;
